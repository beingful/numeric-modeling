...
class TwoStepSymmetricAlgorithm:
    ...
    def __calculate_implicit(self, t_step, x_step, y_step):
        ...
        f_x_y_t_next = self.__f.calculate(self.__x0 + x_step * self.__hx,
                                          self.__y0 + y_step * self.__hy,
                                          self.__t0 + (t_step + 1) * self.__tau)

        return ((w_x_y_t + self.__tau *
                 (f_x_y_t_next + self.__w.a *
                  ((w_x_next_y_t_next + w_x_prev_y_t_next) / self.__hx ** 2
                   + (w_x_y_next_t_next + w_x_y_prev_t_next) / self.__hy ** 2)))
                / (1 + 2 * self.__tau * (1 / self.__hx ** 2 + 1 / self.__hy ** 2)))
